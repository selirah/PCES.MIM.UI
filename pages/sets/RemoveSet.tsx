...
const RemoveSet: React.FC<Props> = (props) => {
  const { openDelete, setOpenDelete, setId } = props;
  ...
  const deleteHandler = async () => {
    try {
      await axios.put(
        ENV.NEXT_PUBLIC_API_ENDPOINT + `/Set/deactivateSetBySetId?setId=${setId}`
      );
      setSuccessMessage('Successfully Removed Set');
      setShowSuccess(true);
    } catch (error) {
      console.log(error);
      setShowError(true);
  ...
  return openDelete ? (
    <>
      ...
      {showSuccess ? (
        <GeneralSuccessPopup
          successMessage={`Successfully Removed Set`}
          buttonLink={'/'}
          buttonMessage={'Ok'}
          onClick={onClick}
        />
