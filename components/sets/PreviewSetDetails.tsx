...
const PreviewSetDetails: React.FC<Props> = (props: Props) => {
  const { data } = props;
  ...
  const [setExpirationDate, setSetExpirationDate] = useState<string>();
  ...
  useEffect(() => {
    if (data != undefined) {
      ...
      setSetExpirationDate(data.expirationDate.split("T")[0])
    }
  }, [data]);
      ...
      <div className="p-4">
        <div className="w-full flex justify-between  ">
          <div className="w-full justify-between shadow-md rounded-md mt-4 border-t-2 m-2 p-4">
            ...
            <div className="mt-2 flex-col flex">
              <label className="font-semibold text-sm">Expiration Date</label>
              <input
                type="text"
                className="rounded-md bg-gray-100"
        ...
        <PreviewModel setModelId={`${selectedModelId}`} />
      </div>
    </>
  );
};

export default PreviewSetDetails;
