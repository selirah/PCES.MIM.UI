...
const Arrival: React.FC = () => {
    ...
    try {
      const result = await axios.get(
        ENV.NEXT_PUBLIC_API_ENDPOINT +
          `/Process/wasteArrivalInfo?interventionBarcode=${barcode}`
      );
      return result;
    } catch (error) {
      setShowError(true);
  ...
  const onSubmit = async () => {
    try {
      await axios.put(
        ENV.NEXT_PUBLIC_API_ENDPOINT +
          `/Process/updateWasteArrival?processId=${formData.processId}`
      );
      setShowSuccess(true);
    } catch (error) {
      setShowError(true);
  ...
  useEffect(() => {
    if (data != undefined) {
      ...
      dataToAdd['dateAndTimeOfTransport'] = new Date();
  ...
  return (
    <>
      <div className="m-4">
        ...
        <div className="w-full flex mt-8">
          <div className="flex flex-col w-1/2">
            <div className="shadow-md rounded-xl mb-4">
              <div>
          ...
          <div className="shadow-md rounded-lg w-1/2 ml-4">
            <div>
              <label className="bg-gray-200 text-lg w-full p-2 rounded-t-lg">
                Location
              </label>
      ...
      {showSuccess ? (
        <GeneralSuccessPopup
          successMessage="Waste arrived"
          buttonLink="/waste-treatment"
          buttonMessage="Go To Waste Treatment"
          onClick={onClick}
        />
      ) : (
        ''
