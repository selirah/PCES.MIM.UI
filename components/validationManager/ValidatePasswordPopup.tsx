...
interface Props {
  ...
  validatePasswordResult: (isValid: boolean) => void;
  onClose: any;
}

const ValidatePasswordPopup: React.FC<Props> = (props) => {
  const { openValidate, validatePasswordResult, onClose } = props;
    ...
    <>
      ...
      >
        <div className="rounded-md relative w-full h-auto max-w-screen-sm bg-blend-overlay bg-white">
          <div className='grid justify-items-stretch'><button className="justify-self-end px-6 text-lg font-semibold w-0 h-0" onClick={onClose}>X</button></div>
...
export default ValidatePasswordPopup;
