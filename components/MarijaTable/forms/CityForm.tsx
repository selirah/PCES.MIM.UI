...
const City: React.FC<Props> = (props: Props) => {
  ...
  const handleSubmit = async () => {
    try {
      if (submission?.cityId == undefined) {
        setSuccessMessage('Successfully created city');
        await axios.post(ENV.NEXT_PUBLIC_API_ENDPOINT + '/City', submission);
      } else {
        setSuccessMessage('Successfully updated city');
        await axios.put(ENV.NEXT_PUBLIC_API_ENDPOINT + '/City', submission);
      }
      setShowSuccess(true);
    } catch (error) {
      setShowError(true);
