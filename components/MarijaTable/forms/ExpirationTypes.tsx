...
const ExpirationTypes: React.FC<Props> = (props: Props) => {
  ...
  const handleSubmit = async () => {
    try {
      if (submission?.ExpirationTypesId == undefined) {
        setSuccessMessage('Successfully created Expiration Types');
        await axios.post(
          ENV.NEXT_PUBLIC_API_ENDPOINT + '/Set/addExpirationTypes',
          submission
        );
      } else {
        setSuccessMessage('Successfully updated Expiration Types');
        await axios.put(
          ENV.NEXT_PUBLIC_API_ENDPOINT + '/Set/expirationTypes',
          submission
        );
      }
      setShowSuccess(true);
      ...
      <div className="w-full text-center">
        <div className="flex justify-between mb-3">
          ...
          <input
            ...
            onChange={(event) => {
              setSubmission({
                ExpirationTypesId: submission?.ExpirationTypesId,
